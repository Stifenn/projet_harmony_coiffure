	      ...
	      <?php foreach($Tarifs as $CurrentTarifs) :?>
	      	<tr>
		      	<form  action="<?= $this->url('modifier_tarifs',['id' => $CurrentTarifs['id']])?>" method="POST" accept-charset="utf-8">
			      	...
			      	<td><input class="supTarif" type="submit" name="supprimer" value="Supprimer" /></td>
		      	</form>
			</tr>
	      <?php endforeach ?>
	      <tr>
	     	 <form action="<?= $this->url('ajouter_tarifs')?>" method="POST" accept-charset="utf-8">
		      	...
		      	<td><input type="text" name="prix_hommes" placeholder="prix pour les hommes" /></td>
	      		<td><input type="text" name="prix_femmes" placeholder="prix pour les femmes" /></td>
